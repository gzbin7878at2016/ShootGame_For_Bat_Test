...
USTRUCT()
struct FRequest_Heartbeat {
	GENERATED_BODY()
public:
	UPROPERTY()
		FString op = TEXT("heartbeat");
	UPROPERTY()
		FString account = TEXT("");
	UPROPERTY()
		int timestamp = 0;
...
protected:
	...
	void Heartbeat(FRequest_Heartbeat st);
	void HeartbeatResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful);

	void TalkResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful);

	void talkToServer();

	void findBatTestParam();
	...
	// 
	bool bBatTestMode;
	FString BatTestServer;
	FString BatTestClient;
	bool bBatTestRun;
	FString HttpUrl;
	FString OpenIpPort;
	FHttpModule* pHttp_;
