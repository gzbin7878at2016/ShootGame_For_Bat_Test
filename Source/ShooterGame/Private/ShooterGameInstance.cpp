...
{
	UE_LOG(LogOnline, Log, TEXT("==OnPreLoadMap==%s"), *MapName);
...
{
	UE_LOG(LogOnline, Log, TEXT("==OnPostLoadMap=="));
...
void UShooterGameInstance::MaybeChangeState()
{
	...
	{
		UE_LOG(LogOnlineGame, Log, TEXT("==start MaybeChangeState=CurrentState=%s, PendingState=%s"), *CurrentState.ToString(), *PendingState.ToString());
		...
		UE_LOG(LogOnlineGame, Log, TEXT("==end MaybeChangeState=CurrentState=%s, PendingState=%s"), *CurrentState.ToString(), *PendingState.ToString());
	}
}
...
bool UShooterGameInstance::HostGame(ULocalPlayer* LocalPlayer, const FString& GameType, const FString& InTravelURL)
{
	...
	UE_LOG(LogOnlineGame, Log, TEXT("==host game=type=%s, url=%s, session=%d"), *GameType, *InTravelURL, GameSession);
	if (GameSession)
	{
		// add callback delegate for completion
		OnCreatePresenceSessionCompleteDelegateHandle = GameSession->OnCreatePresenceSessionComplete().AddUObject(this, &UShooterGameInstance::OnCreatePresenceSessionComplete);
		...
		{
			UE_LOG(LogOnlineGame, Log, TEXT("==host game=CurrentState=%s, PendingState=%s"), *CurrentState.ToString(), *PendingState.ToString());
			// If any error occurred in the above, pending state would be set
			if ( (PendingState == CurrentState) || (PendingState == ShooterGameInstanceState::None) )
			{
				// Go ahead and go into loading state now
...
{
	UE_LOG(LogOnline, Log, TEXT("==HandleOpenCommand==cmd=%s"), Cmd);
...
void UShooterGameInstance::GetJsonStringFromStruct(StructType FilledStruct, FString& StringOutput) {
	FJsonObjectConverter::UStructToJsonObjectString(StructType::StaticStruct(), &FilledStruct, StringOutput, 0, 0);
}

template <typename StructType>
void UShooterGameInstance::GetStructFromJsonString(FHttpResponsePtr Response, StructType& StructOutput) {
...
bool UShooterGameInstance::ResponseIsValid(FHttpResponsePtr Response, bool bWasSuccessful) {
...
void UShooterGameInstance::Heartbeat(FRequest_Heartbeat st) {

	FString ContentJsonString;
	GetJsonStringFromStruct<FRequest_Heartbeat>(st, ContentJsonString);
	UE_LOG(LogTemp, Warning, TEXT("==Heartbeat: %s"), *ContentJsonString);
	FRequest_data data;
	data.data = FBase64::Encode(ContentJsonString);
	data.md5 = FMD5::HashAnsiString(*data.data);

	FString ContentJsonString2;
	GetJsonStringFromStruct<FRequest_data>(data, ContentJsonString2);
	UE_LOG(LogTemp, Warning, TEXT("==22Heartbeat: %s"), *ContentJsonString2);
	
	TSharedRef<IHttpRequest> Request = PostRequest("", ContentJsonString2);
	Request->OnProcessRequestComplete().BindUObject(this, &UShooterGameInstance::HeartbeatResponse);
	Send(Request);
}

void UShooterGameInstance::Send(TSharedRef<IHttpRequest>& Request) {
	Request->ProcessRequest();
}

void UShooterGameInstance::HeartbeatResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	if (!ResponseIsValid(Response, bWasSuccessful)) {
		return;
	} 

	FResponse_common response;
	GetStructFromJsonString<FResponse_common>(Response, response);

	UE_LOG(LogTemp, Warning, TEXT("success: %d"), response.success);
	UE_LOG(LogTemp, Warning, TEXT("code: %d"), response.code);
	UE_LOG(LogTemp, Warning, TEXT("data: %s"), *response.data);
}

TSharedRef<IHttpRequest> UShooterGameInstance::RequestWithRoute(FString Subroute) {
	TSharedRef<IHttpRequest> Request = pHttp_->CreateRequest();
	Request->SetURL(HttpUrl + Subroute);
	SetRequestHeaders(Request);
	return Request;
}

void UShooterGameInstance::SetRequestHeaders(TSharedRef<IHttpRequest>& Request) {
	Request->SetHeader(TEXT("User-Agent"), TEXT("X-UnrealEngine-Agent"));
	Request->SetHeader(TEXT("Content-Type"), TEXT("application/json"));
	Request->SetHeader(TEXT("Accepts"), TEXT("application/json"));
}

TSharedRef<IHttpRequest> UShooterGameInstance::GetRequest(FString Subroute) {
	TSharedRef<IHttpRequest> Request = RequestWithRoute(Subroute);
	Request->SetVerb("GET");
	return Request;
}

TSharedRef<IHttpRequest> UShooterGameInstance::PostRequest(FString Subroute, FString ContentJsonString) {
	TSharedRef<IHttpRequest> Request = RequestWithRoute(Subroute);
...
void UShooterGameInstance::talkToServer() {
	...
	int now = (int)FDateTime::UtcNow().ToUnixTimestamp();
	if (now - LastTimeSend > 1) {
		LastTimeSend = now;

		UE_LOG(LogTemp, Warning, TEXT("==talkToServer"));
		// 
		FString type = TEXT("");
		FString name = TEXT("");
		FString info = TEXT("");
		if (!BatTestServer.IsEmpty()) {
			type = TEXT("game-server");
			name = BatTestServer;
			info = LocalIpPort;
		}
		else {
			type = TEXT("game-client");
			name = BatTestClient;
			info = BatTestClient;
		}
		...
		if (!type.IsEmpty()) {
			...
			st.info = LocalIpPort;
			FString ContentJsonString;
			GetJsonStringFromStruct<FRequest_talk>(st, ContentJsonString);

			TSharedRef<IHttpRequest> Request = PostRequest("/api/v1/btt/game/talk", ContentJsonString);
			Request->OnProcessRequestComplete().BindUObject(this, &UShooterGameInstance::TalkResponse);
			Send(Request);
			bSending = true;
		}
...
void UShooterGameInstance::TalkResponse(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	bSending = false;
	if (!ResponseIsValid(Response, bWasSuccessful)) {
		return;
	}
	...
	UE_LOG(LogTemp, Warning, TEXT("==TalkResponse success: %d"), response.success);
	UE_LOG(LogTemp, Warning, TEXT("==TalkResponse code: %d"), response.code);
	UE_LOG(LogTemp, Warning, TEXT("==TalkResponse data: %s"), *response.data);
...
void UShooterGameInstance::findBatTestParam()
{
	UE_LOG(LogOnlineGame, Log, TEXT("==start find btt params=="));
	if (!FCommandLine::IsInitialized())
	{
		UE_LOG(LogOnlineGame, Log, TEXT("==err not init=="));
		return;
	}
	TArray<FString> tokenList;
	TArray<FString> switchList;
	tokenList.Empty(); switchList.Empty();
	FCommandLine::Parse(FCommandLine::Get(), tokenList, switchList);
	for (int i = 0; i < tokenList.Num(); ++i)
	{
		if (tokenList[i].StartsWith("gamemode=BTT"))
		{
			bBatTestMode = true;
			UE_LOG(LogOnlineGame, Log, TEXT("==gamemode=btt=="));
			continue;
		}
		else if (tokenList[i].StartsWith("server="))
		{
			FString key, value;
			tokenList[i].Split("=", &key, &value);
			BatTestServer = value;
			UE_LOG(LogOnlineGame, Log, TEXT("==btt server==%s"), *BatTestServer);
			continue;
		}
		else if (tokenList[i].StartsWith("client="))
		{
			FString key, value;
			tokenList[i].Split("=", &key, &value);
			BatTestClient = value;
			UE_LOG(LogOnlineGame, Log, TEXT("==btt client==%s"), *BatTestClient);
			continue;
		}
		else if (tokenList[i].StartsWith("open="))
		{
			FString key, value;
			tokenList[i].Split("=", &key, &value);
			OpenIpPort = value;
			UE_LOG(LogOnlineGame, Log, TEXT("==btt OpenIpPort==%s"), *OpenIpPort);
			continue;
		}
		else if (tokenList[i].StartsWith("http="))
		{
			FString key, value;
			tokenList[i].Split("=", &key, &value);
			HttpUrl = value;
			UE_LOG(LogOnlineGame, Log, TEXT("==btt HttpUrl==%s"), *HttpUrl);
			continue;
		}
		else if (tokenList[i].StartsWith("local="))
		{
			FString key, value;
			tokenList[i].Split("=", &key, &value);
			LocalIpPort = value;
			UE_LOG(LogOnlineGame, Log, TEXT("==btt LocalIpPort==%s"), *LocalIpPort);
			continue;
		}
	}
	UE_LOG(LogOnlineGame, Log, TEXT("==end=="));
}


void UShooterGameInstance::StartBatTestOneTime() {
	if (CurrentState != ShooterGameInstanceState::MainMenu) {
		//UE_LOG(LogOnlineGame, Log, TEXT("==not main menu=="));
		return;
	}

	if (bBatTestRun) {
		//UE_LOG(LogOnlineGame, Log, TEXT("==has run btt=="));
		return;
	}
	...
	if (bBatTestMode) {
		if (!BatTestServer.IsEmpty()) {
			bBatTestRun = true;
			UE_LOG(LogOnlineGame, Log, TEXT("==Bat Server start=="));
			// HOST GAME
			SetOnlineMode(EOnlineMode::Online);

			const FString GameType = "BTT";
			const FString MapName = "Sanctuary";
			const FString BotsCountOptionName = "Bots";
			const int32 BotsCountOpt = 0;
			...
			FString const StartURL = FString::Printf(TEXT("/Game/Maps/%s?game=%s%s%s?%s=%d%s"),
				...
				*BotsCountOptionName,
				BotsCountOpt,
				TEXT(""));
			HostGame(GetFirstGamePlayer(), "BTT", StartURL);

		}
		else if (!BatTestClient.IsEmpty()) {
			// JOIN GAME
			bBatTestRun = true;
			UE_LOG(LogOnlineGame, Log, TEXT("==Bat Client start==OpenIpPort=%s"), *OpenIpPort);
			GEngine->Exec(GetWorld(), TEXT("stat fps"));
			// 
			HandleOpenCommand(*OpenIpPort, *GetGlobalLogSingleton(), GetWorld());
		}
	}
}